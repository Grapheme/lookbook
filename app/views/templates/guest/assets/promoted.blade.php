...
<div style="background-image: url('{{ $background }}');" class="item__cont">
    <div class="post-card">
        <div class="item__date">
            <a href="{{ pageurl($categories[$promoted_post['category_id']]['slug']) }}" class="post-photo__alt">
                {{ @$categories[$promoted_post['category_id']]['title'] }}
            </a>
            <a href="{{ URL::route('post.public.show',array($promoted_post['category_id'].'-'.BaseController::stringTranslite($categories[$promoted_post['category_id']]['slug']),$promoted_post['id'].'-'.BaseController::stringTranslite($promoted_post['title']))) }}">
